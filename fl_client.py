...
class LocalModel(object):
    def __init__(self, model_config, data_collected, cid):
        # model_config:
        ...
        #print ("@@@@@",self.y_train[5:10][:])

        if cid == '-1':
        ##if cid == '0' or cid == '1' or cid == '2' or cid == '3':
            self.xx_train, self.yy_train = fake_datasource.generate_fake_MNIST(train_data,test_data)
            # self.xx_train = np.random.rand(800,32,32,3)
            # y=np.zeros(800)
            # y[:400]=0.9
            # self.yy_train = keras.utils.to_categorical(y, 10)
    ...
    def train_one_round(self,r,cid):
        self.model.compile(loss=keras.losses.categorical_crossentropy,
              optimizer=keras.optimizers.Adadelta(),
              metrics=['accuracy'])
        ...
        #if r == 4 and cid == '0':
        if cid == '-1':
            self.model.fit(self.xx_train, self.yy_train,
                    epochs=self.model_config['epoch_per_round'],
                    batch_size=self.model_config['batch_size'],
                    verbose=1,
                    validation_data=(self.x_valid, self.y_valid))
            score = self.model.evaluate(self.xx_train, self.yy_train, verbose=0)
        else:
    ...
    #     if cid == '-1':
    #         self.model.fit_generator(datagen.flow(self.xx_train, self.yy_train,
    #             batch_size=self.model_config['batch_size']),
    #             steps_per_epoch=100,
    #             epochs=5,
    #             validation_data=(self.x_valid, self.y_valid))
...
class FederatedClient(object):
    ...
    def on_init(self, *args):
        ...
        self.local_model = LocalModel(model_config, fake_data, self.cid)
    ...
    def register_handles(self):
        ...
        def on_request_update(*args):
            ...
            my_weights, train_loss, train_accuracy = self.local_model.train_one_round(r,self.cid)
            resp = {
